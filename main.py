...
@app.post("/admin/create-matiere-multi")
async def admin_create_matiere_multi(
    request: Request,
    admin_info: Tuple[str, Dict[str, Any]] = Depends(require_admin),
    db: Session = Depends(get_db)
):
    """Admin creates new matière for multiple filières at once"""
    admin_username, admin_data = admin_info
    is_ajax = request.headers.get("X-Requested-With") == "XMLHttpRequest"
    ...
    try:
        ...
        niveau = form_data.get("niveau")
        semestre = form_data.get("semestre")
        filiere_ids = form_data.getlist("filiere_ids")
        
        # Validation
        if not nom or not code or not niveau or not semestre:
        ...
        # Créer la matière pour chaque filière sélectionnée
        created_count = 0
        skipped_count = 0
        errors = []
        ...
        for filiere_id in filiere_ids:
            ...
            filiere = db.query(FiliereDB).filter(FiliereDB.id == filiere_id).first()
            if not filiere:
                errors.append(f"Filière {filiere_id} non trouvée")
                continue
            ...
            if not admin_data.get("is_main_admin", False):
                ufr = db.query(UFRDB).filter(UFRDB.id == filiere.ufr_id).first()
                if ufr and ufr.universite_id != admin_data.get("universite_id"):
                    errors.append(f"Vous ne pouvez créer des matières que dans votre université (filière: {filiere.nom})")
                    continue
            
            # Vérifier si la matière existe déjà pour cette filière/niveau/semestre
            existing_matiere = db.query(MatiereDB).filter(
                MatiereDB.code == code,
                MatiereDB.filiere_id == filiere_id,
                MatiereDB.niveau == niveau,
        ...
        message_parts = []
        if created_count > 0:
            message_parts.append(f"✅ {created_count} matière(s) créée(s)")
        if skipped_count > 0:
            message_parts.append(f"⚠️ {skipped_count} déjà existante(s)")
        if errors:
        ...
        return RedirectResponse(url=f"/dashboard/admin?success={result_message}", status_code=303)
