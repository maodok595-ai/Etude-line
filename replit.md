...
- **Search Functionality**: Pure frontend, real-time, case-insensitive search across all admin and professor dashboards for various entities. A real-time chapter search is implemented for student and professor dashboards, featuring automatic parent container expansion and visual feedback. **Form Select Filtering (Oct 18, 2025)**: Creation forms for UFR, Filière, and Matière now include live search bars above dropdown menus, enabling instant filtering of parent entities (universities, UFRs, filières) as users type. The `filterSelectOptions()` function hides non-matching options in real-time and automatically resets selection if the chosen option becomes hidden, improving usability when dealing with large lists.
